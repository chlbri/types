...
export type IndexOfArray<T extends readonly unknown[], S extends number[] = []> = T['length'] extends S['length'] ? S[number] : IndexOfArray<T, [S['length'], ...S]>;
type _DivideBy<N extends number, T extends readonly any[]> = T['length'] extends N ? [true] : T extends readonly [...TupleOf<T[number], N>, ...infer U] ? [true, ..._DivideBy<N, U>] : never;
export type DivideTupleLengthBy<N extends number, T extends readonly any[]> = _DivideBy<N, T>['length'];
...
export type TupleOf<T = any, N extends number = number> = N extends N ? number extends N ? T[] : [..._TupleOf<T, N>] : never;
...
export type ReduceArrayByKey<T, K extends string> = ReduceArray<T> extends Record<K, infer V> ? V : never;
