...
describe('#04 => deepClone - Key formatting', () => {
  describe('#04.01 => should format keys using provided function', () => {
    const original = { firstName: 'John', lastName: 'Doe' } as const;
    const formatKey = (key: string) => key.toUpperCase();
    let cloned: any;

    beforeEach(() => {
      cloned = deepClone(original, formatKey);
    });

    it('#04.01.01 => should format firstName to FIRSTNAME', () => {
      expect(cloned.FIRSTNAME).toBe('John');
    });

    it('#04.01.02 => should format lastName to LASTNAME', () => {
      expect(cloned.LASTNAME).toBe('Doe');
    });

    it('#04.01.03 => should remove original keys', () => {
      expect(cloned.firstName).toBeUndefined();
      expect(cloned.lastName).toBeUndefined();
    });
  });

  describe('#04.02 => should format keys in nested objects', () => {
    const original = {
      userInfo: {
        firstName: 'John',
        address: {
    ...
    it('#04.02.01 => should format firstName in nested object', () => {
      expect(cloned.user_info.first_name).toBe('John');
    });

    it('#04.02.02 => should format streetName in deeply nested object', () => {
      expect(cloned.user_info.address.street_name).toBe('Main St');
    });
  });

  describe('#04.03 => should not affect array indices', () => {
    const original = [{ itemName: 'test' }] as any;
    const formatKey = (key: string) => key.toUpperCase();
    let cloned: any;
    ...
    it('#04.03.01 => should maintain array type', () => {
      expect(Array.isArray(cloned)).toBe(true);
    });

    it('#04.03.02 => should format object keys inside array', () => {
      expect(cloned[0].ITEMNAME).toBe('test');
    });
  });

  describe('#04.04 => should handle undefined formatKey function', () => {
    const original = { firstName: 'John', lastName: 'Doe' } as const;
    let cloned: typeof original;

    beforeEach(() => {
      cloned = deepClone(original, undefined);
    });

    it('#04.04.01 => should match the original object', () => {
      expect(cloned).toEqual(original);
    });

    it('#04.04.02 => should create a new instance', () => {
      expect(cloned).not.toBe(original);
    });
  });
});

describe('#05 => formatKeys function', () => {
  describe('#05.01 => should create a curried function', () => {
    const toUpperCase = deepClone.formatKeys((key: string) =>
      key.toUpperCase(),
    );
    const original = { firstName: 'John', lastName: 'Doe' } as const;
    let result: any;
    ...
    beforeEach(() => {
      result = toUpperCase(original);
    });

    it('#05.01.01 => should format firstName to FIRSTNAME', () => {
      expect(result.FIRSTNAME).toBe('John');
    });

    it('#05.01.02 => should format lastName to LASTNAME', () => {
      expect(result.LASTNAME).toBe('Doe');
    });

    it('#05.01.03 => should create a new instance', () => {
      expect(result).not.toBe(original);
    });
  });

  describe('#05.02 => should work with complex transformations', () => {
    const camelToSnake = deepClone.formatKeys((key: string) =>
      key.replace(/([A-Z])/g, '_$1').toLowerCase(),
    );
    const original = {
      firstName: 'John',
    ...
    beforeEach(() => {
      result = camelToSnake(original);
    });

    it('#05.02.01 => should transform top-level keys', () => {
      expect(result.first_name).toBe('John');
      expect(result.last_name).toBe('Doe');
    });

    it('#05.02.02 => should transform nested keys', () => {
      expect(result.user_profile.profile_image).toBe('image.jpg');
      expect(result.user_profile.contact_info.phone_number).toBe(
        '123-456-7890',
      );
    });
  });

  describe('#05.03 => should be accessible via deepClone.formatKeys', () => {
    const toUpperCase = deepClone.formatKeys((key: string) =>
      key.toUpperCase(),
    );
    const original = { test: 'value' } as const;
    let result: any;
    ...
    it('#05.03.01 => should format test to TEST', () => {
...
describe('#07 => deepClone - Performance and memory', () => {
  ...
  describe('#07.06 => should handle formatKey function with edge cases', () => {
    const original = {
    ...
    it('#07.06.01 => should format key with dashes', () => {
      expect(cloned.keywithdashes).toBe('value1');
    });

    it('#07.06.02 => should format key with underscores', () => {
      expect(cloned.keywithunderscores).toBe('value2');
    });

    it('#07.06.03 => should format camel case key', () => {
      expect(cloned.camelcasekey).toBe('value3');
    });

    it('#07.06.04 => should format uppercase key', () => {
      expect(cloned.uppercase).toBe('value4');
    });
  });
});
